...
class Manager:
    ...
    def __init__(self, model, cfg):
        ...
        # lr scheduler
        self.scheduler_steplr = StepLR(self.optimizer,
                                       step_size=1,
                                       gamma=0.1**(1 / cfg.TRAIN.LR_DECAY))
        ...
        # record file
        self.train_record_file = open(
            os.path.join(cfg.DIR.LOGS, 'training.txt'), 'w')
        self.test_record_file = open(os.path.join(cfg.DIR.LOGS, 'testing.txt'),
                                     'w')
    ...
    def unpack_data(self, data):
        ...
        if self.dataset == 'ShapeNet':
            ...
            partial, _ = utils.helpers.seprate_point_cloud(
                gt,
                npoints, [int(npoints * 1 / 4),
                          int(npoints * 3 / 4)],
                fixed_points=None)
        else:
            raise ValueError(
                'No method implemented for this dataset: {:s}'.format(
                    self.dataset))
    ...
    def train(self, model, train_data_loader, val_data_loader, cfg):
        ...
        for epoch_idx in range(init_epoch + 1, cfg.TRAIN.N_EPOCHS + 1):
            ...
            for batch_idx, (taxonomy_ids, model_ids,
                ...
                loss_total, losses, gts = get_loss(pcds_pred,
                                                   partial,
                                                   gt,
                                                   sqrt=True)
                ...
                message = '{:d} {:.4f} {:.4f} {:.4f} {:.4f} {:.4f}'.format(
                    n_itr, cd_pc_item, cd_p1_item, cd_p2_item, cd_p3_item,
                    partial_item)
            ...
            self.train_record(
                ...
                % (epoch_idx, cfg.TRAIN.N_EPOCHS, learning_rate,
                   epoch_end_time - epoch_start_time, [
                       '%.4f' % l for l in
                       [avg_cdc, avg_cd1, avg_cd2, avg_cd3, avg_partial]
                   ]))

            # Validate the current model
            cd_eval = self.validate(cfg,
                                    model=model,
    ...
    def validate(self, cfg, model=None, val_data_loader=None, outdir=None):
        ...
        test_losses = AverageMeter(
            ['cdc', 'cd1', 'cd2', 'cd3', 'partial_matching'])
        test_metrics = AverageMeter('cd3')

        # Start testing
        for model_idx, (taxonomy_id, model_id,
                        data) in enumerate(val_data_loader):
            ...
            with torch.no_grad():
                ...
                loss_total, losses, _ = get_loss(pcds_pred,
                                                 partial,
                                                 gt,
                                                 sqrt=True)
        ...
        message = '#{:d} {:.4f} {:.4f} {:.4f} {:.4f} | {:.4f} | #{:d} {:.4f}'.format(
            self.epoch, test_losses.avg(0), test_losses.avg(1),
            test_losses.avg(2), test_losses.avg(4), test_losses.avg(3),
            self.best_epoch, self.best_metrics)
    ...
    def test_pcn(self, cfg, model=None, test_data_loader=None, outdir=None):
            ...
            with torch.no_grad():
                ...
                if outdir:
                    ...
                    for mm, model_name in enumerate(model_id):
                        output_file = os.path.join(outdir, taxonomy_id,
                                                   model_name)
                        ...
                        write_ply(output_file + '_partial.ply',
                                  ...
                                  ['x', 'y', 'z'])
                        # output img files
                        img_filename = os.path.join(outdir,
                                                    taxonomy_id + '_images',
                                                    model_name + '.jpg')
        ...
        self.train_record('Taxonomy\t#Sample\t' +
                          '\t'.join(test_metrics.items))
