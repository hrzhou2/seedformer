...
from models.utils import vTransformer, PointNet_SA_Module_KNN, MLP_Res, MLP_CONV, fps_subsample, \
...
class FeatureExtractor(nn.Module):
    def __init__(self, out_dim=1024, n_knn=20):
        ...
        self.sa_module_2 = PointNet_SA_Module_KNN(128,
                                                  16,
                                                  128, [128, 256],
                                                  group_all=False,
                                                  if_bn=False,
        ...
        self.sa_module_3 = PointNet_SA_Module_KNN(None,
                                                  None,
                                                  256, [512, out_dim],
                                                  group_all=True,
                                                  if_bn=False)
...
class UpTransformer(nn.Module):
    def __init__(self,
        ...
        self.scale = scale_layer(
            dim=-1) if scale_layer is not None else nn.Identity()
        ...
        if up_factor:
            self.attn_mlp.append(
                nn.ConvTranspose2d(dim * attn_hidden_multiplier,
                                   attn_out_channel, (up_factor, 1),
                                   (up_factor, 1)))
        else:
            self.attn_mlp.append(
        ...
        self.upsample1 = nn.Upsample(
            scale_factor=(up_factor, 1)) if up_factor else nn.Identity()
        self.upsample2 = nn.Upsample(
            scale_factor=up_factor) if up_factor else nn.Identity()
...
class UpLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp_2 = MLP_CONV(in_channel=128 * 2 + seed_dim,
                              layer_dims=[256, dim])

        self.uptrans1 = UpTransformer(dim,
                                      dim,
                                      dim=64,
                                      n_knn=self.n_knn,
                                      use_upfeat=use_upfeat,
                                      up_factor=None)
        self.uptrans2 = UpTransformer(dim,
                                      dim,
                                      dim=64,
                                      n_knn=self.n_knn,
                                      use_upfeat=use_upfeat,
        ...
        self.mlp_delta_feature = MLP_Res(in_dim=dim * 2,
                                         hidden_dim=dim,
                                         out_dim=dim)
    ...
    def forward(self, pcd_prev, seed, seed_feat, K_prev=None):
        ...
        if self.interpolate == 'nearest':
            ...
            feat_upsample = indexing_neighbor(seed_feat, idx).squeeze(
                3)  # (B, seed_dim, N_prev)
        elif self.interpolate == 'three':
            # three interpolate
            idx, dis = get_nearest_index(
                pcd_prev, seed, k=3,
                return_dis=True)  # (B, N_prev, 3), (B, N_prev, 3)
            dist_recip = 1.0 / (dis + 1e-8)
            norm = torch.sum(dist_recip, dim=2, keepdim=True)  # (B, N_prev, 1)
            weight = dist_recip / norm  # (B, N_prev, 3)
            feat_upsample = torch.sum(indexing_neighbor(seed_feat, idx) *
                                      weight.unsqueeze(1),
                                      dim=-1)  # (B, seed_dim, N_prev)
        elif self.interpolate == 'global':
            feat_upsample = seed_feat.repeat((1, 1, n_prev))
        else:
            raise ValueError('Unknown Interpolation: {}'.format(
                self.interpolate))
        ...
        feat_1 = torch.cat([
            ...
            torch.max(feat_1, 2, keepdim=True)[0].repeat(
                (1, 1, feat_1.size(2))), feat_upsample
        ], 1)
        Q = self.mlp_2(feat_1)

        # Upsample Transformers
        H = self.uptrans1(pcd_prev,
                          K_prev if K_prev is not None else Q,
                          Q,
                          upfeat=feat_upsample)  # (B, 128, N_prev)
        ...
        if self.displacement:
            delta = torch.tanh(self.mlp_delta(torch.relu(
                K_curr))) / self.radius**self.i  # (B, 3, N_prev * up_factor)
            pcd_new = self.upsample(pcd_prev)
            pcd_new = pcd_new + delta
        else:
            pcd_new = self.mlp_delta(torch.relu(K_curr))  # relu??
...
class SeedFormer(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp_seed = nn.Conv1d(256, embed_dim, 1)
        self.seed_generator = UpLayer(dim=embed_dim,
                                      seed_dim=feat_dim,
                                      up_factor=seed_factor,
                                      n_knn=n_knn,
                                      radius=radius,
                                      interpolate='global',
                                      displacement=False,
    ...
    def forward(self, partial_cloud):
        ...
        # Decoder
        pred_pcds = self.forward_decoder(feat, partial_cloud, patch_xyz,
                                         patch_feat)
    ...
    def forward_encoder(self, partial_cloud):
        ...
        feat, patch_xyz, patch_feat = self.feat_extractor(
            partial_cloud)  # (B, feat_dim, 1)
    ...
    def forward_decoder(self, feat, partial_cloud, patch_xyz, patch_feat):
        ...
        # Generate Seeds
        patch_feat = self.mlp_seed(patch_feat)
        seed, seed_feat = self.seed_generator(patch_xyz,
                                              seed=None,
                                              seed_feat=feat,
                                              K_prev=patch_feat)
        ...
        # Upsample layers
        pcd = fps_subsample(torch.cat([seed, partial_cloud], 1),
                            self.num_p0)  # (B, num_p0, 3)
        K_prev = None
...
def seedformer_dim128_quickattn(**kwargs):
    model = SeedFormer(feat_dim=512,
                       embed_dim=128,
                       n_knn=20,
                       attn_channel=False,
                       **kwargs)
